...
internal static class InstagramProviderTestFactory
{
    public static (InstagramProvider Provider, FakeHttpMessageHandler Handler) Create(
        Func<HttpRequestMessage, HttpResponseMessage>? responder = null,
        InstagramProviderOptions? options = null)
    {
        var handler = new FakeHttpMessageHandler(responder);
        var httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://graph.facebook.com/") };
        var providerOptions = options ?? new InstagramProviderOptions
        {
            AppId = "app-id",
            AppSecret = "app-secret",
            RedirectUri = "https://localhost/callback",
            DefaultInstagramUserId = "17841412345678901"
        };
        return (new InstagramProvider(httpClient, Options.Create(providerOptions)), handler);
    }

    public static Dictionary<string, string> ParseQuery(Uri uri) => ParseParameters(uri.Query);
